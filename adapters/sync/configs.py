...
def fetch_channel_configs(session: Session, channel_name: str, channel_info: dict, path_configs: str = "v2ray-configs-raw.txt") -> None:
    v2ray_count = 0
    bar_format = " {percentage:3.0f}% |{bar}| {n_fmt}/{total_fmt} "
    range_channel_id = range(channel_info.get("current_id", 0), channel_info.get("last_id", 0), 20)
...
def save_configs(configs: list[dict[str, Any]], path_configs_clean: str = "v2ray-configs-clean.txt", mode: str = "w") -> None:
...
def write_configs(configs: list, path_configs: str = "v2ray-configs-raw.txt", mode: str = "w") -> None:
