...
* **tests/** — каталог со всеми тестами проекта, проверяющими корректность, стабильность и работу модулей
  ...
  * **e2e/** — сквозные тесты, проверяющие полное поведение системы при различных сценариях использования (**не реализовано**)
    ...
    * `test_sync_scraper.py` — проверяет работу синхронного скрейпера с реальными данными
  ...
  * **unit/** — модульные тесты, проверяющие отдельные функции и классы проекта
    ...
    * **adapters/** — тесты адаптеров для работы с данными (**не реализовано**)
      ...
      * **async_/** — асинхронные адаптеры для проверки функций

        * `test_async_channels.py` — проверяет обработку и валидацию каналов

        * `test_async_configs.py` — проверяет обработку и валидацию конфигов

        * `test_async_scraper.py` — проверяет работу асинхронного скрейпера локально

      * **sync/** — синхронные адаптеры для проверки функций

        * `test_sync_channels.py` — проверяет обработку и валидацию каналов

        * `test_sync_configs.py` — проверяет обработку и валидацию конфигов

        * `test_sync_scraper.py` — проверяет работу синхронного скрейпера локально
    ...
    * **domain/** — тесты, проверяющие работу доменной логики проекта (**пропущены, так как логика была изменена и тесты пока не переписаны**).
